...
def train():
    print(f"Starting FC-Last-Layer + k=12 Experiment on {DEVICE}")
    model = GATv2_FCLast(21, 2).to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    train_loader = DataLoader(MediumDataset('train', k=12), batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True)
    val_loader = DataLoader(MediumDataset('val', k=12), batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True)
    
    log_file = os.path.join(PROJECT_DIR, "train_fc_last_k12.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_fc_last_k12_{epoch}.pt")
            epoch += 1
