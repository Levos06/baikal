...
PROJECT_DIR = "2026-03-27_full_mlp_optimized_1500ep"
BATCH_SIZE = 512
NUM_WORKERS = 4
TOTAL_EPOCHS = 1500
...
def evaluate(model, loader, device, criterion, num_batches=100):
    ...
    return total_loss/(i+1), precision_score(all_labels, preds, zero_division=0), recall_score(all_labels, preds, zero_division=0), p9

def train():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Starting SILENT MARATHON on {device} (1500 Epochs, BS=512)")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            t_loss, t_prec, t_rec, t_p9 = evaluate(model, train_loader, device, criterion)
            ...
            if epoch % 100 == 0:
                torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_final_{epoch}.pt")
