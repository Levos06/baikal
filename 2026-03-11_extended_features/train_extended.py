...
class BaikalExtendedDataset(Dataset):
    ...
    def get(self, idx):
        with h5py.File(self.file_path, 'r') as f:
            ...
            # 1-5: Charge, Time, X, Y, Z
            x_main = f['train/data/data'][start:end]
            x, y = x_main[:, 2], x_main[:, 3]
            
            # 6: t_res
            x_tres = (f['train/t_res/data'][start:end] - T_RES_MEAN) / T_RES_STD
            
            # 7-8: R, Phi
            x_r = (np.sqrt(x**2 + y**2) - R_MEAN) / R_STD
            x_phi = (np.arctan2(y, x) - PHI_MEAN) / PHI_STD
            
            num_nodes = x_main.shape[0]
            if num_nodes <= 1:
                edge_index = torch.zeros((2, 0), dtype=torch.long)
                x_deg = np.zeros(num_nodes)
            else:
                indices = np.arange(num_nodes)
                mask = (np.abs(indices[:, None] - indices) <= self.k) & (indices[:, None] != indices)
                edges = np.where(mask)
                edge_index = torch.from_numpy(np.array(edges)).long()
                
                # 9: Degree (нормализованный)
                counts = np.bincount(edges[0], minlength=num_nodes)
                x_deg = (counts - DEG_MEAN) / DEG_STD
            
            # Объединяем всё в (N, 9)
            x_combined = np.column_stack([x_main, x_tres, x_r, x_phi, x_deg])
...
class GCN_Extended(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # 4 слоя: 9 -> 512 -> 768 -> 512 -> 2
...
def train(num_train=200000, num_val=25000, epochs=300, batch_size=512):
    project_dir = "2026-03-11_extended_features"
    plots_dir = os.path.join(project_dir, 'plots')
    checkpoints_dir = os.path.join(project_dir, 'checkpoints')
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device} | Experiment: Extended Features (9) | Events: {num_train}")
    
    train_loader = DataLoader(BaikalExtendedDataset(FILE_PATH, 0, num_train, k=2), batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(BaikalExtendedDataset(FILE_PATH, num_train, num_val, k=2), batch_size=batch_size, num_workers=4)
    train_eval_loader = DataLoader(BaikalExtendedDataset(FILE_PATH, 0, num_val, k=2), batch_size=batch_size, num_workers=4)

    model = GCN_Extended(9, 2).to(device) # 9 input channels
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} | Time: {epoch_time:.1f}s | LR: {current_lr:.2e} | T-Loss: {t_loss:.4f} | V-Loss: {v_loss:.4f} | T-P@R0.9: {t_p9:.4f} | V-P@R0.9: {v_p9:.4f}", flush=True)
        
        scheduler.step(v_loss)
        
        if epoch % 20 == 0:
            torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model_extended.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(plots_dir, 'metrics_extended.png')); plt.close()
