...
class DeepGATv2(torch.nn.Module):
    def __init__(self, in_channels, out_channels, layers=6, heads=4, h_dim=64):
        super().__init__()
...
def train():
    print(f"Starting DEEP GATv2 on {DEVICE} (Batch 256)")
    model = DeepGATv2(21, 2, layers=6, heads=4, h_dim=64).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_deep.log")
    epoch = 1
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 50 == 0:
                torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints_deep/model_deep_{epoch}.pt")
            epoch += 1
