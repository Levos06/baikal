"""
General MPNN + dynamic cosine-similarity edges (layers 2–3), threshold beta on hidden h.
Analog of 2026-05-07_general_mpnn_cos_sim_beta07_k4 with beta=0.5 (more edges than 0.7).
"""
import torch
...
PROJECT_DIR = "/home/levos/experiments/2026-05-11_general_mpnn_cos_sim_beta05_k4"
COS_SIM_BETA = 0.5
BATCH_SIZE = 32  # same as beta=0.7 run (FC + dynamic edges)
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
def get_similarity_edges(h, batch_vector, threshold: float):
    device = h.device
    counts = torch.bincount(batch_vector)
    edge_indices = []
    offset = 0
...
class MPNN_CosSim_FCLast(nn.Module):
    def __init__(self, in_channels, out_channels, cos_threshold: float):
        super().__init__()
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        for i in range(1, 3):
            sim_edges = get_similarity_edges(h, batch_vector, threshold=self.cos_threshold)
...
def train():
    print(
        f"General MPNN + CosSim (beta={COS_SIM_BETA}) + k=4 | {DEVICE} | batch={BATCH_SIZE}"
    )
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_cossim_beta05_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_cossim_beta05_k4_{epoch}.pt",
                )
            epoch += 1
