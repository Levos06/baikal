...
def train():
    ...
    log_file = os.path.join(PROJECT_DIR, "train_auxiliary.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if S is not None and mask is not None:
            # Auxiliary task: S_ij should predict y_i * y_j
            y_dense, _ = to_dense_batch(batch.y.float(), batch.batch) # (B, N_max)
            y_target = torch.bmm(y_dense.unsqueeze(2), y_dense.unsqueeze(1)) # (B, N_max, N_max)
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_aux_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
