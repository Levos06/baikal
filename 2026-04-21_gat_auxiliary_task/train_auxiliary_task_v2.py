...
PROJECT_DIR = "/home/levos/experiments/2026-04-21_gat_auxiliary_task"
BATCH_SIZE = 128
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
class GATv2_DynamicInput(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Projections for Q, K on input features
    ...
    def forward(self, x, edge_index, batch_vector):
        if self.current_epoch > 0: # Start rewiring from epoch 1 immediately!
            Q = self.proj_Q(x)
            K = self.proj_K(x)
            Q_dense, mask = to_dense_batch(Q, batch_vector)
            K_dense, _ = to_dense_batch(K, batch_vector)
            edge_index = rewire_edges(edge_index, batch_vector, Q_dense, K_dense, mask, alpha=ALPHA)
...
def evaluate(model, loader, num_batches=50):
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            out, _, _ = model(batch.x, batch.edge_index, batch.batch)
...
def train():
    print(f"Starting Auxiliary Task Rewiring Experiment on {DEVICE}")
    model = GATv2_DynamicInput(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_auxiliary_v2.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if S is not None and mask is not None:
            # Auxiliary task: S_ij should predict 1 if y_i == y_j else 0
            y_dense, _ = to_dense_batch(batch.y.float(), batch.batch) # (B, N_max)
            y_target = (y_dense.unsqueeze(2) == y_dense.unsqueeze(1)).float() # (B, N_max, N_max)
            ...
            loss_aux = F.binary_cross_entropy_with_logits(S_flat, y_target_flat)
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_aux_v2_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
