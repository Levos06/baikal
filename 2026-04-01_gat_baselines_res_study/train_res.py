...
class GATv2_Res_NoJK(torch.nn.Module):
    def __init__(self, in_channels, out_channels, layers=4, heads=4, h_dim=64):
        super().__init__()
        ...
        # MLP Head (input only from LAST layer)
        self.head = torch.nn.Sequential(
            torch.nn.Linear(h_dim * heads, 1024), torch.nn.GELU(),
    ...
    def forward(self, x, edge_index):
        ...
        for conv, proj in zip(self.convs, self.projs):
            h = F.gelu(conv(h, edge_index) + proj(h))
        return self.head(h)
...
def train():
    print(f"Starting GATv2 RES (No JK) on {DEVICE}")
    model = GATv2_Res_NoJK(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_res.log")
    epoch = 1
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 50 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints_res/model_res_{epoch}.pt")
            epoch += 1
