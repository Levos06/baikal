...
class GATv2_NoRes_NoJK(torch.nn.Module):
...
def train():
    print(f"Starting GATv2 NO-RES (No JK) on {DEVICE}")
    model = GATv2_NoRes_NoJK(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_nores.log")
    epoch = 1
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 50 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints_nores/model_nores_{epoch}.pt")
            epoch += 1
