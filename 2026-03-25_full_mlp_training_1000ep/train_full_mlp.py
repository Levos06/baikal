...
PROJECT_DIR = "2026-03-25_full_mlp_training_1000ep"
BATCH_SIZE = 128
...
def train():
    ...
    print(f"Device: {device} | FULL MLP | OPTIMIZED DATA LOADING")
    ...
    # Resume Logic
    start_epoch = 1
    ckpts = glob.glob(f"{PROJECT_DIR}/checkpoints/model_full_mlp_*.pt")
    if ckpts:
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % ACCUMULATION_STEPS == 0:
            ...
            if (i // ACCUMULATION_STEPS + 1) % VIRTUAL_EPOCH_SIZE == 0:
                ...
                print(f"Epoch {epoch:04d} | Time: {epoch_time:.1f}s | Opt: Multiprocessing (n=4)")
                ...
                if epoch % 50 == 0:
                    torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_full_mlp_{epoch}.pt")
