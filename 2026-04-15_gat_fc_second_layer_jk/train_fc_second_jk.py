...
PROJECT_DIR = "/home/levos/experiments/2026-04-15_gat_fc_second_layer_jk"
...
class GATv2_FCSecond_JK(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        
        # Layer 1: Standard GATv2 (Local Connectivity)
        self.conv1 = GATv2Conv(in_channels, 64, heads=4)
        self.proj1 = torch.nn.Linear(in_channels, 256)
        
        # Layer 2: GATv2 with Complete Graph (Global Connectivity)
        self.fc_conv = GATv2Conv(256, 64, heads=4)
        
        # Layers 3-4: Standard GATv2 (Local Connectivity)
        self.conv3 = GATv2Conv(256, 64, heads=4)
        self.conv4 = GATv2Conv(256, 64, heads=4)
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        # Layer 2 (Global / Fully Connected)
        fc_edge_index = get_complete_edge_index(batch_vector)
        h2 = F.gelu(self.fc_conv(h1, fc_edge_index) + h1) # Residual skip
        
        # Layer 3 (Local)
        h3 = F.gelu(self.conv3(h2, edge_index) + h2)
        
        # Layer 4 (Local)
        h4 = F.gelu(self.conv4(h3, edge_index) + h3)
        
        # Jumping Knowledge: Concat outputs of layer 2 and layer 4
        h_jk = torch.cat([h2, h4], dim=-1)
        
        return self.head(h_jk)
...
def train():
    print(f"Starting FC-Second-Layer + JK Experiment on {DEVICE}")
    model = GATv2_FCSecond_JK(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_fc_second_jk.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_fc_second_jk_{epoch}.pt")
            epoch += 1
