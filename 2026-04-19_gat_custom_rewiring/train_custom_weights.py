...
def rewire_edges_custom_weights(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=0.1):
    ...
    # Calculate Custom Differentiable Weights
    # W_ij = S_ij (raw similarity score, no sigmoid needed as requested by the user, we will just pass S_ij directly)
    # The formula is e_ij = a^T LeakyReLU(W[h_i || h_j]) + W_ij
    row, col = edge_index
    edge_weights = (Q[row] * K[col]).sum(dim=-1).unsqueeze(1) # [E, 1], raw S_ij
    
    return edge_index, edge_weights

class CustomGAT_DiffWeights(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Global Layer (Standard GATv2Conv is fine since it doesn't use dynamic edge_weights)
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        if self.current_epoch > 1:
            ...
            edge_index, edge_weights = rewire_edges_custom_weights(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=ALPHA)
        else:
            # First epoch: no rewiring. W_ij = 0 so it doesn't affect attention logits.
...
def train():
    print(f"Starting Custom GATv2 Diff-Weights Rewiring Experiment on {DEVICE}")
    model = CustomGAT_DiffWeights(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_custom_weights.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_custom_weights_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
