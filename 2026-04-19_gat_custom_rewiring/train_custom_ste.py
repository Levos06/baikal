...
class CustomGATv2Conv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_weight=None, return_attention_weights=False):
        ...
        # Propagate messages (passes x_l, x_r, and edge_weight)
    ...
    def message(self, x_j, x_i, edge_weight, index, ptr, size_i):
        ...
        if edge_weight is not None:
            # edge_weight is [E, 1], add it to all heads
...
def rewire_edges_custom_ste(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=0.1):
    ...
    # Calculate Custom STE Weights
    # For STE, we want the forward pass to be exactly 0 (meaning W_ij has no effect in forward, it's a "hard" graph structure),
    # but in the backward pass, we want gradients to flow to S_ij.
    # Since the formula is e_ij = a^T LeakyReLU(...) + W_ij, if W_ij = 0 in forward, it's a standard GAT.
    
    row, col = edge_index
    edge_scores = (Q[row] * K[col]).sum(dim=-1).unsqueeze(1) # S_ij
    
    # We apply a soft transformation (e.g. sigmoid) just to bound the gradients nicely
    edge_soft = torch.sigmoid(edge_scores)
    
    # STE: Forward pass evaluates to 0.0. Backward pass evaluates to grad(edge_soft).
    edge_weights = (0.0 - edge_soft).detach() + edge_soft
    
    return edge_index, edge_weights

class CustomGAT_DiffSTE(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        if self.current_epoch > 1:
            ...
            edge_index, edge_weights = rewire_edges_custom_ste(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=ALPHA)
        else:
...
def train():
    print(f"Starting Custom GATv2 Diff-STE Rewiring Experiment on {DEVICE}")
    model = CustomGAT_DiffSTE(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_custom_ste.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_custom_ste_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
