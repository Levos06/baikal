...
PROJECT_DIR = "/home/levos/experiments/2026-04-19_gat_dynamic_input_k2"
BATCH_SIZE = 128
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
def train():
    print(f"Starting Dynamic Input Edge Rewiring Experiment on {DEVICE}")
    ...
    log_file = os.path.join(PROJECT_DIR, "train_dynamic_input.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_dynamic_input_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
