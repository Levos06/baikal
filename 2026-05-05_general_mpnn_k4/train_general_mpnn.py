...
PROJECT_DIR = "/home/levos/experiments/2026-05-05_general_mpnn_k4"
...
class MPNN_FCLast(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
...
def train():
    print(f"Starting General MPNN (Mean Aggr) + k=4 Experiment on {DEVICE}")
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_mpnn_k4_{epoch}.pt")
            epoch += 1
