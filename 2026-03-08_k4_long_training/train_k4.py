...
def train(num_train=150000, num_val=20000, epochs=300, batch_size=512):
    project_dir = "2026-03-08_k4_long_training"
    ...
    print(f"Device: {device} | Experiment: k=4 Long | Events: {num_train}")
    
    # K=4 ИСПОЛЬЗУЕТСЯ ТУТ
    K_VAL = 4
    train_loader = DataLoader(BaikalTemporalGraphDataset(FILE_PATH, 0, num_train, k=K_VAL), batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(BaikalTemporalGraphDataset(FILE_PATH, num_train, num_val, k=K_VAL), batch_size=batch_size, num_workers=4)
    train_eval_loader = DataLoader(BaikalTemporalGraphDataset(FILE_PATH, 0, num_val, k=K_VAL), batch_size=batch_size, num_workers=4)

    model = GCN_Capacity(5, 512, 2).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model_k4_long.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(plots_dir, 'metrics_k4_long.png')); plt.close()
