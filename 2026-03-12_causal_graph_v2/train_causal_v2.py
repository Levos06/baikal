...
THRESHOLD = 0.5
V_WATER = 0.225
T_CUT = 3.0     # Improved: much tighter causal window
D_MAX = 100.0
...
class BaikalCausalV2Dataset(Dataset):
...
class GCN_CausalV2(torch.nn.Module):
...
def train(num_train=100000, num_val=15000, epochs=300, batch_size=256):
    project_dir = "2026-03-12_causal_graph_v2"
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device} | Experiment: Causal Graph v2 (T_CUT=3.0) | Events: {num_train}", flush=True)
    
    train_loader = DataLoader(BaikalCausalV2Dataset(FILE_PATH, 0, num_train), batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(BaikalCausalV2Dataset(FILE_PATH, num_train, num_val), batch_size=batch_size, num_workers=4)
    train_eval_loader = DataLoader(BaikalCausalV2Dataset(FILE_PATH, 0, num_val), batch_size=batch_size, num_workers=4)

    model = GCN_CausalV2(13, 2).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), os.path.join(project_dir, 'checkpoints/model_causal_v2.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(project_dir, 'plots/metrics_causal_v2.png')); plt.close()
