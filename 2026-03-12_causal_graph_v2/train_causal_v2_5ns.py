...
THRESHOLD = 0.5
V_WATER = 0.225
T_CUT = 5.0     # Changed to 5.0 ns for parallel experiment
D_MAX = 100.0
...
def train(num_train=100000, num_val=15000, epochs=300, batch_size=256):
    ...
    print(f"Device: {device} | Experiment: Causal Graph 5ns | Events: {num_train}", flush=True)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), os.path.join(project_dir, 'checkpoints/model_causal_5ns.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(project_dir, 'plots/metrics_causal_5ns.png')); plt.close()
