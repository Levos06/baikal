...
def run_study():
    t_cuts = list(range(1, 16))
    results = []
    num_train, num_val = 100000, 15000
    epochs = 10
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"Starting ASYMMETRIC NO-TRES Study on {device}...")
    for t_cut in t_cuts:
        print(f"\n--- Testing T_CUT_half = {t_cut} ns ---")
        train_loader = DataLoader(BaikalNoTresDataset(FILE_PATH, 0, num_train, t_cut=t_cut), batch_size=256, shuffle=True, num_workers=4)
        val_loader = DataLoader(BaikalNoTresDataset(FILE_PATH, num_train, num_val, t_cut=t_cut), batch_size=256, num_workers=4)
        
        model = GCN_V3(12, 2).to(device)
        ...
        print(f"Final V-P@R0.9 for T_CUT_half={t_cut}: {final_p9:.4f}")
        results.append(final_p9)
        
        plt.figure(figsize=(10, 6))
        plt.plot(t_cuts[:len(results)], results, marker='o', color='brown')
        plt.title("Honest Causal: P@R0.9 vs T_CUT")
        plt.xlabel("T_CUT half-window (ns)")
        plt.ylabel("Validation P@R0.9")
        plt.grid(True)
        plt.savefig("2026-03-13_causal_window_study/plots/study_honest_asymmetric_progress.png")
        plt.close()

    print("\nHonest Study complete!")
    print("T_CUTs:", t_cuts)
