...
class BaikalSymmetricDataset(Dataset):
...
def run_study():
    ...
    print(f"Starting SYMMETRIC Study on {device}...")
    for t_cut in t_cuts:
        print(f"\n--- Testing SYMMETRIC T_CUT = {t_cut:.1f} ns ---")
        train_loader = DataLoader(BaikalSymmetricDataset(FILE_PATH, 0, num_train, t_cut=t_cut), batch_size=256, shuffle=True, num_workers=4)
        val_loader = DataLoader(BaikalSymmetricDataset(FILE_PATH, num_train, num_val, t_cut=t_cut), batch_size=256, num_workers=4)
        ...
        print(f"Final V-P@R0.9 for Symmetric T_CUT={t_cut:.1f}: {final_p9:.4f}")
        results.append(final_p9)
        
        plt.figure(figsize=(10, 6))
        plt.plot(t_cuts[:len(results)], results, marker='o')
        plt.title("Symmetric: P@R0.9 vs T_CUT")
        plt.xlabel("T_CUT (ns)")
        plt.ylabel("Validation P@R0.9")
        plt.grid(True)
        plt.savefig("2026-03-13_causal_window_study/plots/study_symmetric_progress.png")
        plt.close()

    print("\nSymmetric Study complete!")
    print("T_CUTs:", t_cuts)
