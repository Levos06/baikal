...
class BaikalAsymmetricDataset(Dataset):
    def __init__(self, file_path, start_ev=0, num_events=1000, t_cut=0.01):
        super().__init__()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if batch.edge_index.numel() == 0:
                ...
                out = model(batch.x, torch.zeros((2, 0), dtype=torch.long).to(device))
            else:
...
def run_study():
    t_cuts = [0.0001, 0.0005, 0.001]
    results = []
    num_train, num_val = 100000, 15000
    epochs = 10
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"Starting ASYMMETRIC ULTRA-FINE Study on {device}...")
    for t_cut in t_cuts:
        print(f"\n--- Testing ULTRA-FINE T_CUT_half = {t_cut:.4f} ns ---")
        train_loader = DataLoader(BaikalAsymmetricDataset(FILE_PATH, 0, num_train, t_cut=t_cut), batch_size=256, shuffle=True, num_workers=4)
        val_loader = DataLoader(BaikalAsymmetricDataset(FILE_PATH, num_train, num_val, t_cut=t_cut), batch_size=256, num_workers=4)
        ...
        print(f"Final V-P@R0.9 for Ultra-Fine T_CUT_half={t_cut:.4f}: {final_p9:.4f}")
        results.append(final_p9)
        
        plt.figure(figsize=(10, 6))
        plt.plot(t_cuts[:len(results)], results, marker='o', color='purple')
        plt.title("Ultra-Fine: P@R0.9 vs T_CUT")
        plt.xlabel("T_CUT half-window (ns)")
        plt.ylabel("Validation P@R0.9")
        plt.xscale('log')
        plt.grid(True)
        plt.savefig("2026-03-13_causal_window_study/plots/ultra_fine/study_ultra_fine_progress.png")
        plt.close()

    print("\nUltra-Fine Study complete!")
    print("T_CUTs:", t_cuts)
