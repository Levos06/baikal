    ...
    def get(self, idx):
        with h5py.File(self.file_path, 'r') as f:
            ...
            if num_nodes <= 1:
                ...
                # Асимметричное условие: 0 < (dt - dist/v) < 2*T_CUT
...
def run_study():
    t_cuts = [0.5, 0.8, 1.1, 1.4, 1.7, 2.0, 2.3, 2.6, 2.9, 3.2, 3.5]
    results = []
    num_train, num_val = 100000, 15000
    epochs = 10
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print(f"Starting ASYMMETRIC Study on {device}...")
    for t_cut in t_cuts:
        print(f"\n--- Testing ASYMMETRIC T_CUT_half = {t_cut:.1f} ns (Full Window={2*t_cut:.1f}) ---")
        ...
        print(f"Final V-P@R0.9 for Asymmetric T_CUT_half={t_cut:.1f}: {final_p9:.4f}")
        results.append(final_p9)
        
        plt.figure(figsize=(10, 6))
        plt.plot(t_cuts[:len(results)], results, marker='o', color='red')
        plt.title("Asymmetric: P@R0.9 vs T_CUT")
        plt.xlabel("T_CUT half-window (ns)")
        plt.ylabel("Validation P@R0.9")
        plt.grid(True)
        plt.savefig("2026-03-13_causal_window_study/plots/study_asymmetric_progress.png")
        plt.close()

    print("\nAsymmetric Study complete!")
    print("T_CUTs:", t_cuts)
