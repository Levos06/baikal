...
def rewire_edges_ste(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=0.1):
    ...
    edge_soft = torch.sigmoid(edge_scores).unsqueeze(1) # [E, 1]
    
    # Forward: exactly 1.0 (hard edge). Backward: gradient through edge_soft.
    edge_weights = (1.0 - edge_soft).detach() + edge_soft
    
    return edge_index, edge_weights

class GATv2_DiffSTE(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        if self.current_epoch > 1:
            ...
            edge_index, edge_weights = rewire_edges_ste(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, alpha=ALPHA)
        else:
...
def train():
    print(f"Starting STE Rewiring Experiment on {DEVICE}")
    model = GATv2_DiffSTE(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_ste.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_ste_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
