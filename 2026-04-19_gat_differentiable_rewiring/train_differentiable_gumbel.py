...
def rewire_edges_gumbel(edge_index, batch_vector, Q, K, Q_dense, K_dense, mask, current_epoch, max_epochs, alpha=0.1):
    ...
    edge_weights = torch.sigmoid(edge_scores / tau).unsqueeze(1) # [E, 1]
    
    return edge_index, edge_weights

class GATv2_DiffGumbel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        if self.current_epoch > 1:
            ...
            edge_index, edge_weights = rewire_edges_gumbel(
...
def train():
    print(f"Starting Gumbel-Softmax Rewiring Experiment on {DEVICE}")
    model = GATv2_DiffGumbel(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_gumbel.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_gumbel_{epoch}.pt")
            epoch += 1
            model.current_epoch = epoch
