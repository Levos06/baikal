...
CHECKPOINT = "2026-03-28_extended_21features/checkpoints/model_21feat_norm_800.pt"
...
def add_features_21(batch):
    ...
    event_t_min, event_t_max = scatter(x[:, 1], b_idx, dim=0, reduce='min'), scatter(x[:, 1], b_idx, dim=0, reduce='max')
...
def gradient_importance(model, device, loader):
    ...
    for i, batch in enumerate(loader):
        ...
        out = model(x_f, batch.edge_index)
        # Gradient of the signal class probability w.r.t input features
        prob = torch.sigmoid(out[:, 1]).mean()
        prob.backward()
        grad_acc += x_f.grad.abs().mean(dim=0)
        if (i+1)%10==0: print(f"  Batch {i+1}/50 done")
...
if __name__ == "__main__":
    ...
    val_files = sorted(glob.glob(os.path.join("data_processed_50k/val", "*.pt")))[:2]
    data_list = []
    for f in val_files: data_list.extend(torch.load(f, weights_only=False))
    loader = DataLoader(data_list, batch_size=512)
    
    print("Starting Deep Importance Analysis (21 Features)...")
    ...
    plt.barh(range(21), [perms[i] for i in idx], color='skyblue')
    plt.yticks(range(21), [FEATURE_NAMES[i] for i in idx])
    plt.title('Global Importance (Permutation Drop)')
    
    plt.subplot(1, 2, 2)
    idx_g = np.argsort(grads)
    plt.barh(range(21), [grads[i] for i in idx_g], color='salmon')
    plt.yticks(range(21), [FEATURE_NAMES[i] for i in idx_g])
    plt.title('Local Sensitivity (Mean |Gradient|)')
    
    plt.tight_layout()
    plt.savefig('2026-03-28_extended_21features/plots/deep_importance_21.png')
    print("\nAnalysis complete. Results in deep_importance_21.png")
