...
CHECKPOINT = "2026-03-28_extended_21features/checkpoints/model_21feat_norm_1500.pt"
...
if __name__ == "__main__":
    ...
    print(f"Final Importance Analysis (1500 Epochs)...")
    baseline = run_evaluation(model, device, loader)
    print(f"Baseline P@R0.9: {baseline:.4f}")
    
    results_txt = os.path.join("2026-03-28_extended_21features/plots/importance_results_1500.txt")
    with open(results_txt, 'w') as f_out:
        f_out.write(f"Baseline P@R0.9: {baseline:.4f}\n\n")
        ...
        for i in range(21):
            ...
            drop = baseline - p9
            perms.append(drop)
            line = f"Feature {FEATURE_NAMES[i]:15s} | Drop: {drop:.4f}"
            print(line)
            f_out.write(line + "\n")
    ...
    plt.title(f'FINAL Feature Importance: model_1500.pt (P@R0.9={baseline:.4f})')
    plt.grid(axis='x', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig('2026-03-28_extended_21features/plots/deep_importance_21_final_1500.png')
    print(f"\nAnalysis complete. Results in {results_txt}")
