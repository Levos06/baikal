"""
General MPNN + mean master vertex on all four layers + learnable 3x8 edge embeddings.
Layers 1-3: temporal k=4 hit-hit + bidirectional master<->hits star.
Layer 4: fully connected hit-hit + bidirectional master<->hits star (master kept).
Edge attr for ALL edges: 3 scalars (dt, dr, s2) embedded via per-layer learnable
3 x EDGE_VEC_DIM vectors -> 24-D flat concat (same as edge3_learn8).
Master tzxy = mean of hits tzxy, so geom_edge_attr is well-defined for master-star edges.
"""
import torch
...
PROJECT_DIR = "/home/levos/experiments/2026-06-29_mpnn_master_edge3_learn8_k4"
BATCH_SIZE = 48
NUM_WORKERS = 4
TOTAL_EPOCHS = 2667
...
def geom_edge_attr(tzxy: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
    """3-D edge features (dt, dr, s2) from tzxy cols 0..3. Works for both hit-hit and master-star edges."""
...
def get_fc_edge_index_with_master(batch_hits: torch.Tensor, num_real: int) -> torch.Tensor:
    """FC hit-hit within events + bidirectional master<->hits (master index = num_real + event_id)."""
...
def append_master_vertex(batch):
    """Add one master node per event (mean of hit features). Returns x_full, ei_full, batch_hits, num_real."""
    x = batch.x
    b_idx = batch.batch
    ei = batch.edge_index
    device = x.device
...
class GeneralMPNNConvEdgeProductEmb(MessagePassing):
    """Message MLP sees [x_i, x_j, flat_k(e_k * w_k)] with per-layer learnable w_k in R^EDGE_VEC_DIM."""
    ...
    def embed_edges(self, edge_attr: torch.Tensor) -> torch.Tensor:
        e = edge_attr[:, :EDGE_NUM_SCALARS].unsqueeze(-1) * self.edge_vecs.unsqueeze(0)
        return e.reshape(edge_attr.size(0), EDGE_MSG_DIM)
...
class MPNN_MasterEdge3Learn8(nn.Module):
    """Mean master vertex on all 4 layers + learnable 3x8 edge embeddings for all edges."""

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.convs = nn.ModuleList()
        self.projs = nn.ModuleList()
        self.convs.append(GeneralMPNNConvEdgeProductEmb(in_channels, 256))
        self.projs.append(nn.Linear(in_channels, 256))
        for _ in range(2):
            self.convs.append(GeneralMPNNConvEdgeProductEmb(256, 256))
            self.projs.append(nn.Identity())
        self.fc_conv = GeneralMPNNConvEdgeProductEmb(256, 256)
    ...
    def forward(self, x_full, edge_index_pre_fc, batch_hits, num_real):
        ...
        h = x_full
        for i in range(3):
            ea = geom_edge_attr(tzxy, edge_index_pre_fc)
...
def train():
    print(
        f"MPNN master (mean) + edge3_learn8 (3x{EDGE_VEC_DIM}={EDGE_MSG_DIM}D), all 4 layers | "
        f"{DEVICE} | batch={BATCH_SIZE} | EDGE_VEC_DIM={EDGE_VEC_DIM}"
    )
    model = MPNN_MasterEdge3Learn8(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_master_edge3_learn8_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_master_edge3_learn8_k4_{epoch}.pt",
                )
            epoch += 1
