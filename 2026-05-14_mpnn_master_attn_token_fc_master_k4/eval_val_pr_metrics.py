#!/usr/bin/env python3
"""
Finite one-pass evaluation on data_processed_50k/val (no separate test shard in this tree).
Reports Precision at target recall (default 0.9 and 0.95) from sklearn PR curve.
"""
import argparse
import glob
import os
...
import train_mpnn_master_attn_token_fc_master as T
...
class ValShardDataset(Dataset):
    """Loads val/*.pt once; each __getitem__ returns one graph with k=4 edges."""
...
def main():
    ...
    ap.add_argument(
        "--ckpt",
        type=str,
        default=os.path.join(EXP, "checkpoints/model_mpnn_master_attn_token_fc_master_k4_900.pt"),
        help="Closest saved epoch before best val P@R0.95 (963); 900 recommended.",
    )
    ...
    m = T.MPNN_MasterAttnToken_FCPlusMaster(21, 2).to(device)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            x_f, ei_f, b_h, n_r = T.append_master_vertex_learnable(batch, m.master_attn)
            out = m(x_f, ei_f, b_h, n_r)
    ...
    print(f"split: val (full), n_samples={len(y)}, graphs_in_dataset={len(ds)}")
