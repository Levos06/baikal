"""
General MPNN + attention master token, FC on layer 2 only.
Layer 1: local k=4 + master star
Layer 2: FC hits + master star (global communication)
Layer 3: local k=4 + master star (master absorbs globally enriched hits)
Layer 4: local k=4 + master star (master distributes back)
"""
import torch
...
PROJECT_DIR = "/home/levos/experiments/2026-06-23_mpnn_fc2_master_k4"
BATCH_SIZE = 48  # FC layer on layer 2 is memory intensive
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
def add_features_21_norm(batch):
    ...
    raw_extra = torch.stack(
        [s2, dt, dr, r_xy, phi, rho, cosTheta, tof, mean_dist_neigh, neigh_charge,
         q_rel_mean, cos_alpha, hits_on_string, z_span, n_hits, duration],
        dim=1,
    )
...
def build_star_edges(batch_hits, num_real, device):
    """Bidirectional star: master <-> each hit."""
    hits = torch.arange(num_real, device=device)
    m_global = (num_real + batch_hits).long()
    return torch.stack(
        [torch.cat([hits, m_global]), torch.cat([m_global, hits])], dim=0
    )


def build_fc_with_star(batch_hits, num_real):
    """FC among hits + bidirectional master star."""
    hh = get_complete_edge_index(batch_hits)
    hits = torch.arange(num_real, device=batch_hits.device)
    m_global = (num_real + batch_hits).long()
    ei_star = torch.stack(
        [torch.cat([hits, m_global]), torch.cat([m_global, hits])], dim=0
    )
...
class MasterAttentionToken(nn.Module):
    """
    Learnable [CLS]-like token + softmax attention readout over hits.
    alpha_i = softmax_i( (W_k x_i)^T q / sqrt{d} ) per event,
    master = token + MLP( sum_i alpha_i W_v x_i )
    """
...
class GeneralMPNNConv(MessagePassing):
    def __init__(self, in_channels, out_channels):
        ...
        self.f = nn.Sequential(
            nn.Linear(2 * in_channels, hidden), nn.LayerNorm(hidden), nn.GELU(),
...
class MPNN_FC2_Master(nn.Module):
    """
    Layer 1: local k=4 + master star
    Layer 2: FC hits + master star  (global communication)
    Layer 3: local k=4 + master star  (master absorbs globally enriched hits)
    ...
    def __init__(self, in_channels, out_channels, attn_dim=ATTN_DIM):
        ...
        self.conv1 = GeneralMPNNConv(in_channels, 256)
        self.proj1 = nn.Linear(in_channels, 256)
        self.conv2 = GeneralMPNNConv(256, 256)
        self.conv3 = GeneralMPNNConv(256, 256)
        self.conv4 = GeneralMPNNConv(256, 256)
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        # Star edges (reused on layers 1, 3, 4)
        ei_star = build_star_edges(batch_vector, num_real, device)
        ei_local_star = torch.cat([edge_index, ei_star], dim=1)

        # Layer 1: local k=4 + master star
        h = F.gelu(self.conv1(x_full, ei_local_star) + self.proj1(x_full))

        # Layer 2: FC hits + master star (global communication)
        ei_fc_star = build_fc_with_star(batch_vector, num_real)
        h = F.gelu(self.conv2(h, ei_fc_star) + h)

        # Layer 3: local k=4 + master star (master absorbs)
        h = F.gelu(self.conv3(h, ei_local_star) + h)

        # Layer 4: local k=4 + master star (master distributes)
        h = F.gelu(self.conv4(h, ei_local_star) + h)
...
def train():
    print(
        f"MPNN FC2+Master (attn token), k=4 | {DEVICE} | batch={BATCH_SIZE} | ATTN_DIM={ATTN_DIM}"
    )
    model = MPNN_FC2_Master(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_fc2_master_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_fc2_master_k4_{epoch}.pt",
                )
            epoch += 1
