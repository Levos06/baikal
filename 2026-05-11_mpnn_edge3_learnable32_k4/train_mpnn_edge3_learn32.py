"""
General MPNN + 3 scalar edge features (dt, dr, s2) with per-feature learnable 32-D vectors.
Edge message input uses concat_k (e_k * w_k) in R^96 instead of raw R^3.
Node features: full 21-d after add_features_21_norm (same as edge_geom baseline).
"""
import torch
...
PROJECT_DIR = "/home/levos/experiments/2026-05-11_mpnn_edge3_learnable32_k4"
BATCH_SIZE = 48
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
EDGE_NUM_SCALARS = 3
EDGE_VEC_DIM = 32
EDGE_MSG_DIM = EDGE_NUM_SCALARS * EDGE_VEC_DIM  # 96
C_WATER = 0.225
...
class GeneralMPNNConvEdgeProductEmb(MessagePassing):
    """
    message MLP sees [x_i, x_j, flat_k ( e_k * w_k )] with w_k in R^32 learnable (per layer).
    """
...
class MPNN_FCLastEdgeProductEmb(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
...
def train():
    print(
        f"MPNN edge (dt,dr,s2) * learnable 3x{EDGE_VEC_DIM} + concat in msg, k=4 | {DEVICE} | batch={BATCH_SIZE}"
    )
    model = MPNN_FCLastEdgeProductEmb(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_edge3_learn32_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_edge3_learn32_k4_{epoch}.pt",
                )
            epoch += 1
