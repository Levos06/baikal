...
def main():
    ...
    dataloaders = create_dataloaders(
        ...
        events_reduction_factor=2,
    )

    train_steps = len(dataloaders["train_dataset"]) // train_params["batch_size"]
    print(f"Full epoch steps: {train_steps}")
    
    # Use config value if provided, else use full epoch
    num_iters = train_params.get("num_train_steps_per_validation", train_steps)
    print(f"Steps per epoch (num_iters): {num_iters}")
