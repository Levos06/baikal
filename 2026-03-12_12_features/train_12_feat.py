...
class Baikal13FeatDataset(Dataset):
    ...
    def get(self, idx):
        with h5py.File(self.file_path, 'r') as f:
            ...
            if num_nodes <= 1:
                ...
                # 12: ToF Residual
                dt = np.abs(t[rows] - t[cols])
                # Note: coordinates in Baikal data are usually in some units, distance is calculated via XYZ
                dr = np.sqrt(np.sum((x_main[rows, 2:5] - x_main[cols, 2:5])**2, axis=1))
                tres_causal = np.abs(dt - dr/0.225)
                
                # 13: NeighQ
                x_tof = np.zeros(num_nodes)
                x_nq = np.zeros(num_nodes)
                for node in range(num_nodes):
                    node_neighbors = cols[rows == node]
                    if len(node_neighbors) > 0:
                        x_tof[node] = np.mean(tres_causal[rows == node])
                        x_nq[node] = np.sum(q[node_neighbors])
...
def train(num_train=200000, num_val=20000, epochs=300, batch_size=512):
    project_dir = "2026-03-12_12_features"
    plots_dir = os.path.join(project_dir, 'plots')
    checkpoints_dir = os.path.join(project_dir, 'checkpoints')
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device} | Experiment: 13 Features | Events: {num_train}", flush=True)
    
    train_loader = DataLoader(Baikal13FeatDataset(FILE_PATH, 0, num_train, k=2), batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(Baikal13FeatDataset(FILE_PATH, num_train, num_val, k=2), batch_size=batch_size, num_workers=4)
    train_eval_loader = DataLoader(Baikal13FeatDataset(FILE_PATH, 0, num_val, k=2), batch_size=batch_size, num_workers=4)

    model = GCN_13(13, 2).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model_13feat.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(plots_dir, 'metrics_13feat.png')); plt.close()
