...
PROJECT_DIR = "/home/levos/experiments/2026-04-02_gat_mechanisms_study"
BATCH_SIZE = 256
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
class GATv2_Res(torch.nn.Module):
...
def train():
    print(f"Starting GATv2 mechanism study on {DEVICE}")
    model = GATv2_Res(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_v2.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 50 == 0:
                torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints_v2/model_v2_{epoch}.pt")
