...
PROJECT_DIR = "/home/levos/experiments/2026-04-23_gat_cos_sim_beta07"
...
def get_similarity_edges(h, batch_vector, threshold=0.7):
    # Calculate cosine similarity matrix
    h_norm = F.normalize(h, p=2, dim=1)
    sim = torch.mm(h_norm, h_norm.t())
    
    # Mask to only allow connections within the same event
    same_batch = batch_vector.unsqueeze(0) == batch_vector.unsqueeze(1)
    mask = (sim > threshold) & same_batch
...
class GATv2_CosSimBeta(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        # Layers 2 and 3 (i=1, 2) use dynamic cos sim graph + base graph
        for i in range(1, 3):
            sim_edges = get_similarity_edges(h, batch_vector, threshold=0.7)
...
class MediumDataset(torch.utils.data.IterableDataset):
    def __init__(self, split='train', k=1):
        super().__init__()
...
def train():
    print(f"Starting FC-Last-Layer + CosSim (beta=0.7) + k=1 Experiment on {DEVICE}")
    model = GATv2_CosSimBeta(21, 2).to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    train_loader = DataLoader(MediumDataset('train', k=1), batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True)
    val_loader = DataLoader(MediumDataset('val', k=1), batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True)
    
    log_file = os.path.join(PROJECT_DIR, "train_cos_sim_beta07.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_cos_sim_beta07_{epoch}.pt")
            epoch += 1
