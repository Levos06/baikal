...
PROJECT_DIR = "/home/levos/experiments/2026-05-09_mpnn_master_vertex_k4"
BATCH_SIZE = 128
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
def append_master_vertex(batch):
    """
    One master node per graph: feature = mean of hit features (after add_features_21_norm).
    Bidirectional edges master <-> each hit. Returns:
      x_full, edge_index_full, batch_hits, num_real
    Layer 4 uses only hits: batch_hits labels first num_real nodes.
    """
    x = batch.x
    b_idx = batch.batch
    ei = batch.edge_index
...
class MPNN_MasterVertex_FCLast(nn.Module):
    """Layers 1–3: temporal k=4 + master star; layer 4: FC on hits only."""
...
def train():
    print(f"General MPNN + master vertex (layers 1–3), k=4, on {DEVICE}")
    model = MPNN_MasterVertex_FCLast(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_master_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_master_k4_{epoch}.pt",
                )
            epoch += 1
