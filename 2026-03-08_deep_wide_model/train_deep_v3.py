...
class GCN_V3(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Архитектура v3: 5 -> 512 -> 768 -> 512 -> 2
...
def train(num_train=250000, num_val=25000, epochs=800, batch_size=512):
    ...
    print(f"Device: {device} | Experiment: v3 (768 internal) | Events: {num_train}")
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=2e-4) # lr в 1.5 раза меньше
    ...
    print(f"Starting v3 training (800 epochs)...", flush=True)
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model_deep_wide_v3.pt'))
            ...
            plt.tight_layout(); plt.savefig(os.path.join(plots_dir, 'metrics_deep_wide_v3.png')); plt.close()
