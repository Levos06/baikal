...
PROJECT_DIR = "/home/levos/experiments/2026-04-15_gat_fc_second_layer"
...
class GATv2_FCSecond(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch_vector):
        # Layer 1 (Local)
        h = F.gelu(self.conv1(x, edge_index) + self.proj1(x))
        ...
        # Layer 3 (Local)
        h = F.gelu(self.conv3(h, edge_index) + h)
        
        # Layer 4 (Local)
        h = F.gelu(self.conv4(h, edge_index) + h)
...
def train():
    print(f"Starting FC-Second-Layer Experiment on {DEVICE}")
    model = GATv2_FCSecond(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_fc_second.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_fc_second_{epoch}.pt")
            epoch += 1
