...
PROJECT_DIR = "/home/levos/experiments/2026-05-07_general_mpnn_cos_sim_beta07_k4"
...
def get_similarity_edges(h, batch_vector, threshold=0.7):
    # Calculate cosine similarity matrix per graph to save memory
    device = h.device
    counts = torch.bincount(batch_vector)
...
class MPNN_CosSim_FCLast(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
...
def train():
    print(f"Starting General MPNN + CosSim (beta=0.7) + k=4 Experiment on {DEVICE}")
    model = MPNN_CosSim_FCLast(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_cossim_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_mpnn_cossim_k4_{epoch}.pt")
            epoch += 1
