...
PROJECT_DIR = "/home/levos/experiments/2026-04-03_gat_wide_last_layer"
BATCH_SIZE = 256
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
class GATv2_WideLast(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Layer 2-3: 256 -> 256 (4x64)
        for _ in range(2):
            self.convs.append(GATv2Conv(256, 64, heads=4))
            self.projs.append(torch.nn.Identity())
            
        # Layer 4 (Wide): 256 -> 1536 (6 heads x 256)
        self.convs.append(GATv2Conv(256, 256, heads=6))
        self.projs.append(torch.nn.Linear(256, 1536))
        
        self.head = torch.nn.Sequential(
            torch.nn.Linear(1536, 1024), torch.nn.GELU(),
...
def train():
    print(f"Starting Wide-Last-Layer (1536) Experiment on {DEVICE}")
    model = GATv2_WideLast(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_1536.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 50 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_1536_{epoch}.pt")
            epoch += 1
