"""
General MPNN + 3 scalar edge features (dt, dr, s2) with per-feature learnable 8-D vectors.
Edge message input uses concat_k (e_k * w_k) in R^24 instead of raw R^3.
...
PROJECT_DIR = "/home/levos/experiments/2026-06-29_mpnn_edge3_learn8_full_k4"
BATCH_SIZE = 48
NUM_WORKERS = 4
TOTAL_EPOCHS = 2667
...
class GeneralMPNNConvEdgeProductEmb(MessagePassing):
    """
    message MLP sees [x_i, x_j, flat_k ( e_k * w_k )] with w_k in R^8 learnable (per layer).
    """
...
def train():
    print(
        f"MPNN edge (dt,dr,s2) * learnable 3x8 + concat in msg, k=4 | {DEVICE} | batch={BATCH_SIZE}"
    )
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_edge3_learn8_full_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_edge3_learn8_full_k4_{epoch}.pt",
                )
            epoch += 1
