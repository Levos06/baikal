...
PROJECT_DIR = "/home/levos/experiments/2026-04-19_gat_dynamic_edges_k2"
BATCH_SIZE = 128
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
def rewire_edges(edge_index, batch_vector, Q_dense, K_dense, mask, alpha=0.1):
    B, N_max, _ = Q_dense.shape
    device = edge_index.device
    N = batch_vector.size(0)
    
    # Calculate similarity scores
    S = torch.bmm(Q_dense, K_dense.transpose(1, 2)) # (B, N_max, N_max)
    
    valid_mask = mask.unsqueeze(2) & mask.unsqueeze(1)
    diag_mask = torch.eye(N_max, device=device).unsqueeze(0).bool()
    
    # S_add sets invalid and diagonal to -inf
    S_add = S.clone()
    S_add[~valid_mask | diag_mask] = -float('inf')
    
    # S_remove sets invalid and diagonal to +inf
    S_remove = S.clone()
    S_remove[~valid_mask | diag_mask] = float('inf')
    
    # We need a mapping from dense indices to flat node indices
    # flat_indices has shape (B, N_max), but only valid mask positions hold actual indices
    ...
    # Count original edges per batch item
...
class GATv2_DynamicEdges(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.convs = torch.nn.ModuleList()
        self.projs = torch.nn.ModuleList()
        
        # Layers 1-3: Standard GATv2
        ...
        # Projections for dynamic edge rewiring
        self.proj_Q = torch.nn.Linear(256, 64)
        self.proj_K = torch.nn.Linear(256, 64)
            
        # Layer 4: Standard GATv2 (but will run on rewired edge_index)
        self.conv_last = GATv2Conv(256, 64, heads=4)
    ...
    def forward(self, x, edge_index, batch_vector):
        ...
        # Rewire edges using similarity and the current edge_index
        new_edge_index = rewire_edges(edge_index, batch_vector, Q_dense, K_dense, mask, alpha=ALPHA)
        
        # Last Layer on new graph
        h = F.gelu(self.conv_last(h, new_edge_index) + h) # Residual skip
...
def train():
    print(f"Starting GATv2 Dynamic Edge Rewiring Experiment on {DEVICE}")
    model = GATv2_DynamicEdges(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_dynamic.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0: torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_dynamic_{epoch}.pt")
            epoch += 1
