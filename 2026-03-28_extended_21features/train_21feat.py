...
PROJECT_DIR = "2026-03-28_extended_21features"
BATCH_SIZE = 512
NUM_WORKERS = 4
TOTAL_EPOCHS = 2500 # Extension to 2500
...
def train():
    ...
    print(f"Starting FINE-TUNING MARATHON on {device} (LR=2e-5)")
    model = JKResGCN_21(21, 2).to(device)
    
    # RESUME LOGIC
    start_epoch = 1
    ckpts = glob.glob(f"{PROJECT_DIR}/checkpoints/*.pt")
    if ckpts:
        latest = max(ckpts, key=os.path.getctime)
        print(f"Resuming from {latest}")
        model.load_state_dict(torch.load(latest, weights_only=True))
        start_epoch = int(re.search(r"norm_(\d+).pt", latest).group(1)) + 1
    
    optimizer = torch.optim.Adam(model.parameters(), lr=2e-5) # NEW LOWER LR
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(model.state_dict(), f"{PROJECT_DIR}/checkpoints/model_21feat_norm_{epoch}.pt")
            epoch += 1
