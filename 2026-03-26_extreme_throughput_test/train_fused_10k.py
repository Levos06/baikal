        ...
        while True:
            ...
            for f in self.files:
                try:
                    ...
                    x, ei, y = chunk['x'], chunk['edge_index'], chunk['y']
                    x_ptr, e_ptr = chunk['x_ptr'], chunk['edge_ptr']
                    
                    for i in range(len(y)):
                        ex = x[x_ptr[i] : x_ptr[i+1]]
                        eei = ei[:, e_ptr[i] : e_ptr[i+1]]
                        
                        # Auto-detector for relative vs global indices
                        if eei.numel() > 0:
                            if eei.max() >= ex.size(0):
                                eei = eei - x_ptr[i] # Relativize if global
                        
                        yield Data(x=ex, edge_index=eei, y=y[i].view(-1))
                    del chunk
...
def train():
    ...
    print(f"Starting REACTION-SPEED Training on {device} (BS={BATCH_SIZE})")
    model = JKResGCN_v2(13, 2).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    criterion = torch.nn.CrossEntropyLoss()
    
    loader = DataLoader(FusedDataset(DATA_DIR), batch_size=BATCH_SIZE, num_workers=2, pin_memory=True)
    
    model.train()
    start_time = time.time(); events_count = 0; batch_idx = 0
    for batch in loader:
        try:
            batch = add_features_gpu(batch.to(device))
