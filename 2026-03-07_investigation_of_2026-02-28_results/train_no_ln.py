...
class BaikalTemporalGraphDataset(Dataset):
    ...
    def get(self, idx):
        with h5py.File(self.file_path, 'r') as f:
            ...
            # Updated: Signal is everything not equal to zero
    ...
    def _get_temporal_edges(self, n, k):
        ...
        # Optimized with broadcasting as suggested
        idx = np.arange(n)
        mask = (np.abs(idx[:, None] - idx) <= k) & (idx[:, None] != idx)
        edge_index = np.array(np.where(mask))
        return torch.from_numpy(edge_index).long()

class GCN_NoLN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super().__init__()
        self.conv1 = GCNConv(in_channels, hidden_channels)
        # self.ln1 = LayerNorm(hidden_channels) # Removed
        self.conv2 = GCNConv(hidden_channels, hidden_channels)
        # self.ln2 = LayerNorm(hidden_channels) # Removed
        self.conv3 = GCNConv(hidden_channels, out_channels)

    def forward(self, x, edge_index):
        x = self.conv1(x, edge_index)
        # x = self.ln1(x) # Removed
        x = F.gelu(x) # Updated: GELU instead of ReLU
        x = F.dropout(x, p=0.1, training=self.training) # Updated: p=0.1
        
        x = self.conv2(x, edge_index)
        # x = self.ln2(x) # Removed
        x = F.gelu(x) # Updated: GELU instead of ReLU
        x = F.dropout(x, p=0.1, training=self.training) # Updated: p=0.1
...
def train(num_events_train=50000, num_events_val=10000, epochs=15, batch_size=512):
    ...
    model = GCN_NoLN(in_channels=5, hidden_channels=128, out_channels=2).to(device)
    ...
    checkpoint_path = os.path.join(checkpoints_dir, 'best_model_k2.pt')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'training_metrics_k2.png'))
...
if __name__ == "__main__":
    ...
    if "--smoke-test" in sys.argv:
        ...
        train(num_events_train=100, num_events_val=50, epochs=1, batch_size=32)
    else:
        train()
