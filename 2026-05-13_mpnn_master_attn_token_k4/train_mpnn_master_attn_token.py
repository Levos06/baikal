"""
General MPNN + master vertex with learnable [CLS]-like token and softmax attention readout (no mean init).
Layers 1–3: temporal k=4 + master<->hits star; layer 4: FC on hits only (master dropped).
"""
import torch
...
PROJECT_DIR = "/home/levos/experiments/2026-05-13_mpnn_master_attn_token_k4"
BATCH_SIZE = 128
NUM_WORKERS = 4
TOTAL_EPOCHS = 1000
...
class MasterAttentionToken(nn.Module):
    """
    Learnable embedding e in R^{F} plus cross-graph readout:
    alpha_i = softmax_i( (W_k x_i)^T q / sqrt{d} ) per event,
    master_g = e + MLP( sum_i alpha_i W_v x_i ).
    Distribution to hits: unchanged from baseline (MPNN messages on master<->hit star edges).
    """
...
class MPNN_MasterAttnToken_FCLast(nn.Module):
    ...
    def forward(self, x_full, edge_index_full, batch_hits, num_real):
        ...
        h_hits = h[:num_real]
        fc_ei = get_complete_edge_index(batch_hits)
        h_hits = F.gelu(self.fc_conv(h_hits, fc_ei) + h_hits)
        return self.head(h_hits)
...
def train():
    print(
        f"General MPNN + master attention token ([CLS]+softmax pool), k=4, ATTN_DIM={ATTN_DIM}, on {DEVICE}"
    )
    model = MPNN_MasterAttnToken_FCLast(21, 2).to(DEVICE)
    ...
    log_file = os.path.join(PROJECT_DIR, "train_mpnn_master_attn_token_k4.log")
    ...
    for i, batch in enumerate(train_loader):
        ...
        if (i + 1) % VIRTUAL_EPOCH_SIZE == 0:
            ...
            if epoch % 100 == 0:
                torch.save(
                    model.state_dict(),
                    f"{PROJECT_DIR}/checkpoints/model_mpnn_master_attn_token_k4_{epoch}.pt",
                )
            epoch += 1
